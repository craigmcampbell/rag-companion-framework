"""
ingest.py — Index your Obsidian vault into ChromaDB

Usage:
    python ingest.py --vault /path/to/your/vault
    python ingest.py --vault /path/to/your/vault --reset   # wipe + re-index
...
def iter_markdown_files(vault_path: Path):
    """Yield all .md files under vault_path, skipping hidden dirs."""
...
def parse_frontmatter(text: str) -> tuple[dict, str]:
    ...
    List values are stored as comma-separated strings for ChromaDB compatibility.
    """
    meta = {}
...
def stable_id(file_path: Path, chunk_index: int) -> str:
    """Deterministic ID so re-runs can upsert cleanly."""
    return hashlib.md5(f"{file_path}::{chunk_index}".encode()).hexdigest()


# ── Main ──────────────────────────────────────────────────────────────────────

def ingest(vault_path: Path, reset: bool = False):
    ...
    # First pass: collect every frontmatter key across the vault so we can
    # default-fill missing fields on every chunk (ChromaDB filter requirement)
    ...
    for md_file in md_files:
        ...
        try:
            ...
            embeddings    = model.encode(titled_chunks, show_progress_bar=False).tolist()
    ...
    print(f"DB now contains {collection.count()} total chunks")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reset", action="store_true", help="Wipe the DB before indexing")
